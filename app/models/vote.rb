class Vote < ActiveRecord::Base
  ...
  def self.calculate_vote(voteable, user, value)
    if self.check_user(voteable, user)
      target_vote = voteable.votes.where(:user_id => user).first
      target_vote.update_attributes(:vote_value => target_vote.vote_value + value)
    else
     voteable.votes.create(:user_id=> user, :vote_value => value)
    end
